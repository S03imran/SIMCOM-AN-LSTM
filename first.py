...
if __name__ == '__main__':
    ...
    def create_lstm_model(input_shape):
        model = Sequential([
            LSTM(64, input_shape=(input_shape[1], input_shape[2])),  # Assuming input_shape is (batch_size, sequence_length, input_dim)
    ...
    def train_and_evaluate_lstm(X_train, y_train, X_test, y_test):
        ...
        model = create_lstm_model(X_train_reshaped.shape)
        model.fit(X_train, y_train, epochs=10, batch_size=64, validation_data=(X_test, y_test))
        loss, accuracy = model.evaluate(X_test, y_test)
        print("Test Loss:", loss)
        print("Test Accuracy:", accuracy)

    def prepare_lstm_data(adjacency_matrix, sequence_length=5, test_size=0.2, random_state=None):
    # Initialize lists to store sequences and labels
        sequences = []
        labels = []

        # Iterate over the adjacency matrix to generate sequences and labels
        num_nodes = adjacency_matrix.shape[0]
        for i in range(num_nodes):
            for j in range(num_nodes):
                if i != j and adjacency_matrix[i, j] == 1:
                    # Find interactions for the current node pair within the sequence length
                    current_sequence = []
                    for t in range(sequence_length):
                        if i+t+1 < num_nodes and adjacency_matrix[i+t, j] == 1:
                            current_sequence.append(1)
                        else:
                            current_sequence.append(0)
                    sequences.append(current_sequence)
                    # Generate label indicating whether a link exists in the next time step
                    if i + sequence_length < num_nodes and adjacency_matrix[i + sequence_length, j] == 1:
                        labels.append(1)
                    else:
                        labels.append(0)
